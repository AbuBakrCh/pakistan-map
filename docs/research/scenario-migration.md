...
Every heading, table row, bolded field and `⚠ Footnote required` marker in the draft was read
...
| **A4** 300 km to a capital | "computed" | Rule engine, `distance-to-capital: 300`, 10 units, spread 65.3:1, Gwadar–Quetta at 635 km re-measured by the suite | Carried |
| **A5** Constitutional regularisation | GB (10) and AJK (10) become provinces; announced 1 November 2020 by PM Imran Khan; "draft 26th Amendment"; GBLA resolution; opposed by India; LoC treatment identical | Both units `proposed`, holding exactly their own districts; the announcement, the drafted amendment and the assembly resolution in `status`; India first on the opposition line; a footnote on the dashed line; and a footnote the draft has no equivalent of, saying the two halves are **not equally sourced** | Carried; the amendment's number dropped — see Drift |
...
### The six footnotes the draft marked required

Every `⚠ Footnote required` marker is a card obligation, and every one is met. They are asserted
against the committed bundle in `scripts/lib/bundle.test.ts` — keyed on the footnote *kind* and on
a phrase the draft demanded be said, since a footnote of the right kind saying something else
passes a kind check perfectly.

| Draft | Where it lives now |
|---|---|
| L1 — sources say "11 districts", the pre-2022 count | `district-count` footnote on L1 |
| L4 — the movement names "six districts" | `district-count` footnote on L4 |
| L5 — urban Sindh reaches Hyderabad, Sukkur and Mirpur Khas; no published district list | `omission` footnote on L5 |
| L6 — the boundary is data-determined, not transcribed | `derived-boundary` footnote on L6 |
| H2 — Amb and Phulra omitted, and why | `omission` footnote on H2 |
| H2 — attach no modern population figures | `statistics.modernFigures: false` with its reason, which the scorecard prints where the population lines would be |
...
4. **H1 is three units, not one; H3 is eight, not six.** The draft counted the units its prose was
   about. H1 draws AJK and the Gilgit Agency as themselves, because neither was inside the province
   of West Pakistan; H3 carries ICT and AJK through as well. Every variant is a complete partition
   (D6), so a count that ignores the carried-through units is a count of something else.
...
6. **L3's "95.5–97.8% Pashto-speaking" is superseded by 98.0%.** The draft quoted the two
   present-day halves of South Waziristan. Both fold into the one district this map draws
   (ADR-0001), and the figure for that district, read off Table 11, is 98.0%. The population,
   888,675, is unchanged.
...
it required*: the seventeen ids as a set, and the five required footnotes keyed on kind and on a
phrase, plus H2's withholding and its reason. A footnote deleted from `variants.ts` fails there,
naming the variant, rather than being found by somebody re-reading a document.
