...
describe('regionRoster under a variant', () => {
  for (const variant of scenarios.variants) {
    ...
    it(`says of every ${variant.id} unit which of the three kinds it is`, () => {
      for (const item of roster.items) {
        expect(item.standing, item.name).toMatch(
          /^(Proposed province — not official|Territory, unchanged — not constitutionally a province|Unchanged from the current map)$/,
        );
      }
    });
  }
  ...
  it('says "unchanged" out loud rather than leaving it to inference', () => {
    ...
    expect(sindh?.standing).toBe('Unchanged from the current map');
  });
  ...
  it('names a proposed province as proposed, and only a proposed one', () => {
    ...
    const roster = regionRoster(firstLevel, units);
    const proposed = roster.items.filter((r) => r.standing.startsWith('Proposed'));
