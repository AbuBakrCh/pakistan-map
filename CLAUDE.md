...
| Population | PBS 2023 Digital Census | District level. Extracted from the `PakPC2023` `.RData` tables, committed as upstream bytes in `data/raw/pakpc2023-*.RData` and parsed by `scripts/lib/rdata.ts`, so the numbers trace to a published file rather than to a transcription. Reconciled against PBS Census-2023 **Table 1 (national)** — all 5 provinces, all 31 divisions and the 241,499,431 national total agree exactly |
...
| `scripts/join-census.ts` | `build:data:census` | Reads the committed `PakPC2023` `.RData` cache → resolves census spellings onto the roster → reconciles district sums against published division, province and national totals → `data/bundle/statistics.json`. Fails on an unplaced row, an uncovered district, or a total that does not add up |
